...
def initial():
    ...
    for data_set in config.TRAIN.dataset_list:
        ...
        elif data_set == "LGI-PPGI":
            train_loader.append(data_loader.LGI_PPGILoader.LGI_PPGILoader)
        elif data_set == "ECG-Fitness":
            train_loader.append(data_loader.ECG_FitnessLoader.ECG_FitnessLoader)
        else:
    ...
    for valdata_set in config.VALID.DATA.valdataset_list:
        ...
        elif valdata_set =="LGI-PPGI":
            valid_loader.append(data_loader.LGI_PPGILoader.LGI_PPGILoader)
        elif valdata_set == "ECG-Fitness":
            valid_loader.append(data_loader.ECG_FitnessLoader.ECG_FitnessLoader)
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                                     SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS and iBVP")
    ...
    if (len(config.TRAIN.datapath_list) and len(config.TRAIN.dataset_list)):
        for i in range(config.TRAIN.num_users):
            ...
            train_data_loader = train_loader[i](
                ...
                config_data=config.TRAIN.DATA,i=config.TRAIN.savepath_list[i])
            ...
            valid_data = valid_loader[i](
                ...
                config_data=config.VALID.DATA,i=config.VALID.DATA.savepath_list[i])
        ...
        datadict_list =[None]
    global_loader=data_loader.UBFCPHYSLoader.UBFCPHYSLoader
    global_datadict =  dict()
    valid_data = global_loader(
        name="valid",
        data_path=config.VALID.DATA.globaldatapath,
        config_data=config.VALID.DATA, i=config.VALID.DATA.globalsavepath)
    global_datadict["valid"] = DataLoader(
        dataset=valid_data,
        num_workers=0,
        batch_size=config.TRAIN.BATCH_SIZE,  # batch size for val is the same as train
        shuffle=False,
    ...
    return datadict_list,config,global_datadict
