...
from rPPG_file.evaluation.metrics import calculate_metrics
from rPPG_file.neural_methods.loss.NegPearsonLoss import Neg_Pearson
from rPPG_file.neural_methods.model.PhysNet import PhysNet_padding_Encoder_Decoder_MAX
...
class PhysnetTrainer(BaseTrainer):
    ...
    # def Fed_train(self, data_loader,epotch,state_dict,i,j):
    ...
    #             tbar.set_description("全局第%s轮:第%s个用户:Train epoch %s" %(j+1,i+1,epoch))
    ...
    #     return self.model.state_dict(),mean_training_losses,mean_valid_losses
    #
    ...
    def valid_global(self, data_loader,state):
        """ Runs the model on valid sets."""
        if data_loader["valid"] is None:
            raise ValueError("No data for valid")
    ...
    def Fed_test(self, data_loader,state):
        ...
        # if self.config.TEST.OUTPUT_SAVE_DIR:  # saving test outputs
        #     self.save_test_outputs(predictions, labels, self.config)
        return _,MAE,RMSE,MAPE,correlation_coefficient,SNR,MACC_avg
