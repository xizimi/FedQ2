...
import pandas as pd
import torch
from rPPG_file.evaluation.post_process import *
from tqdm import tqdm
from rPPG_file.evaluation.BlandAltmanPy import BlandAltman
...
def calculate_metrics(predictions, labels, config):
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        for metric in config.TEST.METRICS:
            if metric == "MAE":
                MAE = np.mean(np.abs(predict_hr_peak_all - gt_hr_peak_all))
                ...
                MAPE = np.mean(np.abs((predict_hr_peak_all - gt_hr_peak_all) / gt_hr_peak_all)) * 100
        ...
        raise ValueError("Inference evaluation method name wrong!")
    return MAE,RMSE,MAPE,correlation_coefficient,SNR,MACC_avg


###################################

def Kurtosis_calculate_metrics(predictions, labels, config):
    """Calculate rPPG Metrics (MAE, RMSE, MAPE, Pearson Coef.)."""
    predict_hr_fft_all = list()
    gt_hr_fft_all = list()
    predict_hr_peak_all = list()
    ...
    for index in tqdm(predictions.keys(), ncols=80):
        ...
        for i in range(0, len(prediction), window_frame_size):
            ...
            if config.INFERENCE.EVALUATION_METHOD == "peak detection":
                ...
                gt_hr_fft, pred_hr_fft, SNR, SNR2,macc = snr_calculate_metric_per_video(
                    pred_window, label_window, diff_flag=diff_flag_test, fs=config.TEST.DATA.FS, hr_method='FFT')
                gt_hr_fft_all.append(gt_hr_fft)
                predict_hr_fft_all.append(pred_hr_fft)
                SNR_all.append(SNR)
                ...
                raise ValueError("Inference evaluation method name wrong!")
            Kurtosis_all.append(kurtosis(pred_window.numpy(),fisher=False))
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        raise ValueError("Inference evaluation method name wrong!")
    return MAE, RMSE, MAPE, correlation_coefficient, SNR, MACC_avg,np.mean(Kurtosis_all),np.mean(SNR2_all)