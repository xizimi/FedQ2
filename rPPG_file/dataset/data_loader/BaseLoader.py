...
from rPPG_file.unsupervised_methods import utils
from rPPG_file.unsupervised_methods.methods import POS_WANG
...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data,i):
        ...
        self.cached_path = i
        self.file_list_path = os.path.join(i, 'DataFileLists')
    ...
    def face_detection(self, frame, backend, use_larger_box=False, larger_box_coef=1.0):
        ...
        if backend == "HC":
            ...
            './rPPG_file/dataset/haarcascade_frontalface_default.xml')
    ...
    def save_multi_process(self, frames_clips, bvps_clips, filename):
        ...
        for i in range(len(bvps_clips)):
            assert (len(self.inputs) == len(self.labels))
            input_path_name = self.cached_path  + "{0}_input{1}.npy".format(filename, str(count))
            label_path_name = self.cached_path  + "{0}_label{1}.npy".format(filename, str(count))
